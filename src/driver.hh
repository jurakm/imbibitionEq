...
void driver(GV const& gv, Params params)  // take a copy of params
{
  ...
  MBE mbe(9);
  // <<<5>>>
  typedef Dune::PDELab::GridOperator<GFS,GFS,SLOP,MBE,Real,Real,Real,CC,CC> GO0;
...
//  Dune::PDELab::Alexander2Parameter<Real> method;               // second order
  ...
  Dune::PDELab::FilenameHelper fn(filenm);
  {
    ...
    if(params.vtkout){
       ...
       vtkwriter.addVertexData(new Dune::PDELab::VTKGridFunctionAdapter<DGF>(udgf, filenm));
  ...
  while ( !timeMng.done() ) {
	  ...
	  std::cout << "dt changed : old " << timeMng.dt << ", new: " << dt_applied << "\n";
...
//      std::cout << "(t, int_bdry(grad u)) = (" << timeMng.time <<"," << std::setprecision(12) << intOverBoundary << ")\n";
 ...
 //     std::cout << "t = " << timeMng.time << " (dt = " << timeMng.dt << ")\n";
    ...
    std::vector< std::pair<double,double> > volume_values_der(volume_values.size()); // (t, d/dt int S(t))
	...
	if(std::abs(time - bdry_values[i].first) >= 1.0E-12)
	  throw std::runtime_error(std::string("Time error! i = ") +std::to_string(i)+ " " + std::to_string(time)
	   + " " + std::to_string(bdry_values[i].first));

        out1 << std::setw(10) << std::setprecision(4) << time << " "  // time
             << std::setw(12) << std::setprecision(6) << params.poro * volume_values_der[i].second;  //  " = d/dt int S"
        if(params.model == Model::nonlinear)
